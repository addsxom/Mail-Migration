...
class ServiceDetailsDialog(QDialog):
    def __init__(self, details, parent=None):
        ...
        fields = [
            ...
            ("Traces totales", str(details.get("count", 0))),
            ("Comptes associés", str(len(details.get("accounts", [])))),
        ...
        accounts_title = QLabel("Comptes associés")
        accounts_title.setStyleSheet("font-weight: 700; font-size: 14px; margin-top: 6px;")
        card_layout.addWidget(accounts_title)

        for account in details.get("accounts", []):
            line = QLabel(f"●  {account['email']}    —    {account['count']} traces")
...
class ServicesPage(QWidget):
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "Service", "Catégorie", "Confiance", "Traces", "Compte(s)"
        ])
    ...
    def update_live_detection(self, account_id, data):
        ...
        service_key = data.get("service_id") or data.get("name", "").strip().lower()
        email = data.get("account_email") or self.live_account_emails.get(account_id, "")
        item = self.live_rows.get(service_key)
        ...
        account = item["accounts"].setdefault(account_id, {"email": email, "count": 0})
        account["email"] = email
        account["count"] = max(account["count"], int(data.get("count", 0)))
        item["count"] = sum(account["count"] for account in item["accounts"].values())
        item["score"] = max(item["score"], float(data.get("score", 0)))
        item["last_detected_at"] = data.get("last_detected_at") or item.get("last_detected_at")
        self.live_rows[service_key] = item
        self._render_live_rows()
    ...
    def _render_live_rows(self):
        ...
        for item in sorted(
            ...
            key=lambda value: (-value["score"], value["name"].lower()),
        ):
            accounts = list(item["accounts"].values())
            account_label = (
                f"{len(accounts)} compte(s)"
                if len(accounts) > 1
                else (accounts[0]["email"] if accounts else "—")
            )
            rows.append((
                item["name"], item["category"], f'{item["score"]:.0f} %',
                str(item["count"]), account_label,
            ))
            detail = dict(item)
    ...
    def refresh(self):
        ...
        groups = defaultdict(lambda: {
            "name": "", "category": "", "score": 0, "count": 0,
            "status": "À vérifier", "priority": "Normale", "destination": "",
            "notes": "", "subcategory": "", "first_detected_at": None,
            "last_detected_at": None, "accounts": [],
        })
        ...
        try:
            for account in get_accounts(session):
                ...
                for link in get_account_services(session, account.id):
                    ...
                    group = groups[service.id]
                    group["name"] = service.name
                    group["category"] = service.category
                    group["score"] = max(group["score"], float(link.confidence_score or 0))
                    group["count"] += int(link.trace_count or 0)
                    group["status"] = link.status
                    group["priority"] = link.priority
                    group["destination"] = link.destination_email
                    group["notes"] = link.notes
                    group["subcategory"] = service.subcategory
                    group["first_detected_at"] = link.first_detected_at
                    group["last_detected_at"] = link.last_detected_at
                    group["accounts"].append({
                        "email": account.email,
                        "count": int(link.trace_count or 0),
                    })
        finally:
            session.close()
        ...
        if self.active_account_id is None:
            ...
            self.account_label.setText(
                ...
                if selected_account else "Compte sélectionné introuvable"
            )
        ...
        if not self.live_scan:
            ...
            for group in groups.values():
                ...
                rows.append((
                    ...
                    f'{group["score"]:.0f} %', str(group["count"]), account_label,
                ))
