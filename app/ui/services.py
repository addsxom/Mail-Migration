...
class ServicesPage(QWidget):
    def __init__(self):
        ...
        actions = QHBoxLayout(); actions.addStretch(); self.cleanup_button = QPushButton("🧹 Nettoyage"); self.cleanup_button.setToolTip("Supprimer les résultats issus des scans"); self.cleanup_button.clicked.connect(self.cleanup_scanned_services); actions.addWidget(self.cleanup_button); layout.addLayout(actions)
        self.table = QTableWidget(0, 6); self.table.setHorizontalHeaderLabels(["Compte", "Service", "Catégorie", "Confiance", "Traces", "Statut"]); self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); self.table.setSelectionMode(QTableWidget.NoSelection); self.table.setEditTriggers(QTableWidget.NoEditTriggers); self.table.setFocusPolicy(Qt.NoFocus); self.table.cellDoubleClicked.connect(self._open_details_for_row); layout.addWidget(self.table)
    ...
    def update_live_detection(self, account_id, data):
        ...
        self.live_rows[key] = {"account_id": account_id, "account_email": email, "name": data.get("name", "Service inconnu"), "category": data.get("category", "Autre"), "score": float(data.get("score", 0)), "count": int(data.get("count", 0)), "status": data.get("status", "À vérifier"), "priority": data.get("priority", "Normale"), "destination": data.get("destination_email"), "notes": data.get("notes"), "first_detected_at": data.get("first_detected_at"), "last_detected_at": data.get("last_detected_at")}; self._render_live_rows()
    def finish_live_scan(self, mode):
        if not self.live_scan: return
        if mode == -1: self.keep_live_results_after_cancel(); return
        self.live_scan = False; self.scan_label.setText(""); self.refresh(); self.live_rows.clear(); self.live_account_ids.clear(); self.live_account_emails.clear()
    def keep_live_results_after_cancel(self):
        self.live_scan = False; self.scan_label.setText("Scan annulé — résultats déjà détectés conservés"); self._render_live_rows(); self.live_account_ids.clear(); self.live_account_emails.clear()
    ...
    def _set_rows(self, rows, details=None):
        ...
        for r, row in enumerate(rows):
            for c, value in enumerate(row): item = QTableWidgetItem(str(value)); item.setFlags(item.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable); self.table.setItem(r, c, item)
        ...
        try:
            for account in get_accounts(session):
                ...
                for link in get_account_services(session, account.id):
                    service = link.service; details.append({"account_id": account.id, "account_email": account.email, "name": service.name, "category": service.category, "subcategory": service.subcategory, "score": link.confidence_score, "count": link.trace_count, "status": link.status, "priority": link.priority, "destination": link.destination_email, "notes": link.notes, "first_detected_at": link.first_detected_at, "last_detected_at": link.last_detected_at}); rows.append((account.email, service.name, service.category, f"{link.confidence_score:.0f} %", str(link.trace_count), link.status))
        finally: session.close()
    ...
    def cleanup_scanned_services(self):
        ...
        except Exception as exc: session.rollback(); QMessageBox.critical(self, "Nettoyage", f"Impossible de nettoyer les résultats : {exc}"); return
