...
class ServiceDetailsDialog(QDialog):
    def __init__(self, details, parent=None):
        ...
        self.setModal(True); self.setMinimumWidth(460); self.setMaximumWidth(600)
        root = QVBoxLayout(self); root.setContentsMargins(18, 18, 18, 18); root.setSpacing(12)
        card = QFrame(); card.setObjectName("serviceDetailsCard")
        card.setStyleSheet("""
        ...
        """)
        card_layout = QVBoxLayout(card); card_layout.setContentsMargins(18,18,18,18); card_layout.setSpacing(14)
        ...
        fields = [("Compte Gmail", details.get("account_email", "—")), ("Confiance", self._format_score(details.get("score"))),
                  ("Traces", str(details.get("count", 0))), ("Statut", details.get("status", "À vérifier")),
                  ("Priorité", details.get("priority", "Normale")), ("Destination", details.get("destination", "—")),
                  ...
                  ("Sous-catégorie", details.get("subcategory", "—")), ("Notes", details.get("notes", "—"))]
...
class ServicesPage(QWidget):
    def __init__(self):
        super().__init__(); self.active_account_id = None; self.live_scan = False
        self.live_account_ids = set(); self.live_rows = {}; self.row_details = []; self.live_account_emails = {}
        ...
        self.cleanup_button = QPushButton("🧹 Nettoyage"); self.cleanup_button.setToolTip("Supprimer les résultats issus des scans")
        self.cleanup_button.clicked.connect(self.cleanup_scanned_services); actions.addWidget(self.cleanup_button); layout.addLayout(actions)
        self.table = QTableWidget(0, 6); self.table.setHorizontalHeaderLabels(["Compte", "Service", "Catégorie", "Confiance", "Traces", "Statut"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch); self.table.setSelectionMode(QTableWidget.NoSelection)
        self.table.setEditTriggers(QTableWidget.NoEditTriggers); self.table.setFocusPolicy(Qt.NoFocus); self.table.cellDoubleClicked.connect(self._open_details_for_row)
        layout.addWidget(self.table)

    def set_active_account(self, account_id):
        if self.live_scan: return
        self.active_account_id = account_id; self.live_rows.clear(); self.live_account_ids.clear(); self.live_account_emails.clear(); self.scan_label.setText(""); self.refresh()

    def start_live_scan(self, account_id):
        if not self.live_scan:
            self.live_scan = True; self.live_rows.clear(); self.live_account_ids.clear(); self.live_account_emails.clear()
        ...
        self.scan_label.setText(f"● Scan en cours — {len(self.live_account_ids)} compte(s) — résultats en temps réel"); self._render_live_rows()
    ...
    def update_live_detection(self, account_id, data):
        ...
        self.live_rows[key] = {"account_id": account_id, "account_email": account_email, "name": data.get("name", "Service inconnu"),
            "category": data.get("category", "Autre"), "subcategory": data.get("subcategory"), "score": float(data.get("score", 0)),
            "count": int(data.get("count", 0)), "status": data.get("status", "À vérifier"), "priority": data.get("priority", "Normale"),
            "destination": data.get("destination_email"), "notes": data.get("notes"), "first_detected_at": data.get("first_detected_at"), "last_detected_at": data.get("last_detected_at")}
        self._render_live_rows()

    def finish_live_scan(self, mode):
        if not self.live_scan: return
        if mode == -1:
            self.keep_live_results_after_cancel(); return
        self.live_scan = False; self.scan_label.setText(""); self.refresh()
        self.live_rows.clear(); self.live_account_ids.clear(); self.live_account_emails.clear()

    def keep_live_results_after_cancel(self):
        self.live_scan = False; self.scan_label.setText("Scan annulé — résultats déjà détectés conservés"); self._render_live_rows()
    ...
    def _set_rows(self, rows, details=None):
        self.row_details = details or []; self.table.setRowCount(len(rows))
        for r, row in enumerate(rows):
            for c, value in enumerate(row):
                item = QTableWidgetItem(str(value)); item.setFlags(item.flags() & ~Qt.ItemIsEditable & ~Qt.ItemIsSelectable); self.table.setItem(r, c, item)
    ...
    def refresh(self):
        session = get_session(); rows, details, selected_account = [], [], None
        try:
            for account in get_accounts(session):
                ...
                for link in get_account_services(session, account.id):
                    service = link.service; details.append({"account_id": account.id, "account_email": account.email, "name": service.name,
                        "category": service.category, "subcategory": service.subcategory, "score": link.confidence_score, "count": link.trace_count,
                        "status": link.status, "priority": link.priority, "destination": link.destination_email, "notes": link.notes,
    ...
    def cleanup_scanned_services(self):
        ...
        session = get_session()
        try:
            session.execute(delete(ScanTrace)); session.execute(delete(AccountService)); session.commit()
        except Exception as exc:
            session.rollback(); QMessageBox.critical(self, "Nettoyage", f"Impossible de nettoyer les résultats : {exc}"); return
        self.live_rows.clear(); self.live_account_ids.clear(); self.live_account_emails.clear(); self.live_scan = False; self.scan_label.setText(""); self.refresh()
