...
class SettingsPage(QWidget):
    ...
    def __init__(self):
        ...
        self.palette_label,self.palette=self._combo("Palette de couleurs","Choisissez la palette utilisée par l'application.",["Obsidienne","Ardoise","Minuit","Clair"]); self.palette.setCurrentText(self.values["palette"]); self.palette.currentTextChanged.connect(lambda v:self._set("palette",v)); form.addLayout(self._row(self.palette_label,self.palette))
        self.animations_label,self.animations=self._toggle("Animations","Activer les animations de l'interface.",self.values["animations"]); self.animations.clicked.connect(lambda:self._set("animations",self.animations.isChecked())); form.addLayout(self._row(self.animations_label,self.animations))
        self.toasts_label,self.toasts=self._toggle("Notifications Toast","Afficher les notifications discrètes dans l'interface.",self.values["toast_notifications"]); self.toasts.clicked.connect(lambda:self._set("toast_notifications",self.toasts.isChecked())); form.addLayout(self._row(self.toasts_label,self.toasts))
        self.background_label,self.background=self._toggle("Scanner en arrière-plan","Autoriser les scans à continuer pendant la navigation.",self.values["background_scanner"]); self.background.clicked.connect(lambda:self._set("background_scanner",self.background.isChecked())); form.addLayout(self._row(self.background_label,self.background))
        root.addWidget(card); root.addStretch()
    def _row(self,label,control):
        box=QHBoxLayout(); box.setSpacing(20); box.addWidget(label,1); box.addWidget(control); return box
